...
const ManagerDashboard: React.FC = () => {
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manager dashboard</p>
        </div>
  ...
  const { stats, teams, delegations, events, alerts, recentActivity } = dashboard;
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage your delegation for {displayCountry}
          </p>
        </div>
        ...
        <CardContent>
          {events.length === 0 ? (
            ...
            <div className="space-y-3">
              {events.map(event => (
                <div key={event.id} className="flex items-center justify-between p-4 border rounded-lg gap-4">
                  <div>
                    <h4 className="font-medium">{event.name}</h4>
                    <p className="text-sm text-muted-foreground">
                      {[event.city, formatEventDates(event.startDate, event.endDate)].filter(Boolean).join(' • ')}
                    </p>
                  </div>
